...
const rows = ref<CourseType[]>([])

const isModalOpen = ref(false)
const isEditMode = ref(false)
const editingId = ref<string | null>(null)
const form = ref<CourseType>({ _id: '', code: '', title: '', description: '', sortOrder: 0, isActive: true })

const openCreate = () => {
  isEditMode.value = false
  editingId.value = null
  form.value = { _id: '', code: '', title: '', description: '', sortOrder: rows.value.length + 1, isActive: true }
...
<template>
  <div class="p-6 bg-white dark:bg-gray-900 border-x border-b">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
      <div>
        <h2 class="text-xl font-bold text-gray-950 dark:text-white">انواع دوره</h2>
        <p class="text-sm text-gray-500 mt-1">تعریف و مدیریت انواع دوره‌های آموزشی</p>
      </div>
      <div class="flex flex-wrap gap-2 mt-3 md:mt-0">
        <button @click="openCreate" class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-medium rounded shadow text-sm transition-all">نوع جدید</button>
      </div>
    </div>
    ...
    <div class="overflow-x-auto">
      <TairoTable rounded="sm" class="w-full border">
        ...
        <TairoTableRow v-for="item in rows" :key="item._id">
          <TairoTableCell><code class="ltr-code">{{ item.code }}</code></TairoTableCell>
        ...
        <TairoTableRow v-if="rows.length === 0">
          <TairoTableCell colspan="6" class="py-8 text-center text-gray-400">نوع دوره‌ای ثبت نشده است.</TairoTableCell>
        </TairoTableRow>
      </TairoTable>
    </div>

    <!-- مودال تعریف / ویرایش نوع دوره -->
    <div v-if="isModalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-lg overflow-hidden border dark:border-gray-800">
        <div class="px-6 py-4 border-b dark:border-gray-800">
          <h3 class="text-lg font-bold text-gray-950 dark:text-white">{{ isEditMode ? 'ویرایش نوع دوره' : 'نوع دوره جدید' }}</h3>
        </div>
        <div class="p-6 space-y-4">
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <BaseInput v-model="form.code" :disabled="isEditMode" placeholder="کد" />
            </div>
            <div class="md:col-span-2">
              ...
              <BaseInput v-model="form.title" placeholder="عنوان نوع دوره" />
            </div>
          </div>
          <div>
